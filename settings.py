...
song_default_image = (
    "https://e-cdns-images.dzcdn.net/images/cover/1800x1800-000000-80-0-0.jpg"
)
...
end_keyboard = [[InlineKeyboardButton("SHARE", url=share_message)]]

qualities_keyboard = [
    [
        InlineKeyboardButton(qualities[a], callback_data=qualities[a]),
        InlineKeyboardButton(qualities[a + 1], callback_data=qualities[a + 1]),
    ]
    for a in range(0, len(qualities), 2)
]

first_time_keyboard = [
    [InlineKeyboardButton("Pie Rated Bot", url="t.me/%s?start" % bot_name)]
]

queries = {
    "top": {"query": "%s/top?limit=30", "text": "TOP 30"},
    "albums": {"query": "%s/albums", "text": "ALBUMS"},
    "radio": {"query": "%s/radio", "text": "RADIO"},
    "related": {"query": "%s/related", "text": "RELATED"},
    "download": {"text": "Download"},
    "info": {"text": "Info"},
    "back": {"text": "BACK"},
    "s_art": {"query": "art: %s", "text": "Search by artist"},
    "s_alb": {"query": "alb: %s", "text": "Search by album"},
    "s_pla": {"query": "pla: %s", "text": "Search playlist"},
    "s_lbl": {"query": "lbl: %s", "text": "Search label"},
    "s_trk": {"query": "trk: %s", "text": "Search track"},
    "s_": {"query": "%s", "text": "Global search"},
}
