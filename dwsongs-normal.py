...
def init_user(chat_id, tongue):
    try:
        ...
        users[chat_id] = {"quality": "MP3_320", "tongue": tongue, "c_downloads": 0}
...
def authorized(chat_id, date=0):
    ...
    if ((check_flood(chat_id, date) == "BANNED") or free == 0) and not chat_id in roots:
        ok = False
...
def check_flood(chat_id, time_now=0):
    ...
    try:
        ...
        date[chat_id] = {"time": time_now, "tries": 3, "msg": 0}
    ...
    for a in array:
        keyboard[0] += [
            InlineKeyboardButton(
                "{}: {}".format(a.capitalize(), users[chat_id][a]),
                callback_data="%s" % a,
            )
        ]
...
def sendMessage(chat_id, text, reply_markup=None, reply_to_message_id=None):
    ...
    try:
        ...
        return bot.sendMessage(
            chat_id,
            translate(users[chat_id]["tongue"], text),
...
def sendPhoto(chat_id, photo, caption=None, reply_markup=None):
    ...
    try:
        ...
        return bot.sendPhoto(chat_id, photo, caption=caption, reply_markup=reply_markup)
        ...
        if os.path.isfile(audio):
            ...
            if os.path.getsize(audio) < telegram_audio_api_limit:
                ...
                if not youtube:
                    ...
                    write_db(insert_query % (link, file_id, quality))
            else:
    ...
    if match:
        ...
        try:
            ...
            try:
                ...
                z = mode(link, recursive_download=True, not_interface=not_interface)
...
def Link(link, chat_id, quality, message_id):
    ...
    try:
        if "track/" in link:
            ...
            if any(a in link for a in services_supported):
                sendPhoto(
                    chat_id,
                    image1,
                    caption=(send_image_track_query % (name, artist, album, date)),
                )
            ...
            if any(a in link for a in services_supported):
                ...
                message_id = sendPhoto(
                    chat_id,
                    image1,
                    caption=(send_image_album_query % (name, artist, date, tot)),
                )["message_id"]
                ...
                for a in links:
                    ...
                    exist = c.execute(where_query.format(lins, quali)).fetchone()
            ...
            if "spotify" in link:
                ...
                try:
                    ...
                    if not "The access token expired" in str(a):
                        sendMessage(
                            chat_id, "Invalid link ;)", reply_to_message_id=message_id
                        )
            ...
            if any(a in link for a in services_supported):
                ...
                sendPhoto(
                    chat_id,
                    image1,
                    caption=(send_image_playlist_query % (added, owner, tot)),
                )
            ...
            if any(a in link for a in services_supported[1:]):
                sendPhoto(
                    chat_id,
                    image1,
                    caption=(send_image_artist_query % (artist, albums, fans)),
        ...
        sendMessage(chat_id, "Resend link please...", reply_to_message_id=message_id)
        ...
        sendMessage(
            chat_id,
            "Something went wrong with this link: {} {}, try again".format(
                link, quality
            ),
        )
...
def Audio(audio, chat_id):
    ...
    try:
        ...
        sendMessage(
            chat_id, "File sent is too big, please send a file lower than 20 MB"
        )
        is_audio = 0
        return
    ...
    try:
        ...
        url = reque(api_search_trk % song.replace("#", ""), chat_id, True).json()
    except AttributeError:
        return
    ...
    try:
        ...
        try:
            ids = (
                ...
                % infos["external_metadata"]["spotify"]["track"]["id"]
            )
    ...
    try:
        if query_data in settingss or query_data in qualities:
            ...
            bot.answerCallbackQuery(
                query_id, translate(users[chat_id]["tongue"], "DONE ✅")
            )
            ...
            try:
                url = reque(query_data.replace("down", ""), chat_id, True).json()
            except AttributeError:
                return
            ...
            if "album" in query_data:
                ...
                keyboard.append(
                    [InlineKeyboardButton(queries["back"]["text"], callback_data=link)]
                )
                ...
                bot.answerCallbackQuery(
                    query_id,
                    translate(users[chat_id]["tongue"], "Songs are downloading ⬇️"),
                )
            ...
            if tags[0] == "Infos with too many bytes":
                bot.answerCallbackQuery(
                    query_id, translate(users[chat_id]["tongue"], query_data)
                )

            elif len(tags) == 4:
                bot.answerCallbackQuery(
                    query_id,
                    (tags_query % (tags[0], tags[1], tags[2], tags[3])),
                    show_alert=True,
                )
                ...
                bot.answerCallbackQuery(
                    query_id, translate(users[chat_id]["tongue"], "Song is downloading")
                )

                Link(query_data, chat_id, users[chat_id]["quality"], message_id)
    except TelegramError:
        pass
...
def download(update, context):
    ...
    Thread(
        target=inline, args=(message_id, chat_id, query_data, query_id, tongue)
    ).start()
...
def menu(update, context):
    ...
    if text == "/start":
        ...
        sendMessage(
            chat_id,
            "Settings",
            reply_markup=InlineKeyboardMarkup(create_keyboard(settingss, chat_id)),
        )
        ...
        if chat_id in roots and "the cat is on the table" in text:
            ...
            sendMessage(chat_id, "Press", reply_markup=InlineKeyboardMarkup(keyboard))
            ...
            if ans == "2" and users[chat_id]["c_downloads"] == 3:
                ...
                Thread(
                    target=Link,
                    args=(linked, chat_id, users[chat_id]["quality"], message_id),
                ).start()
...
try:
    ...
    if ans == "1" or ans == "2":
        ...
        sets.dispatcher.add_handler(
            MessageHandler(
                Filters.audio | Filters.text | Filters.voice | Filters.photo, menu
            )
        )
